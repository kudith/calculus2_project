...
export function GeometricSeriesCalculator() {
  ...
  const handleCalculate = () => {
    ...
    if (isNaN(a) || isNaN(r) || isNaN(n) || n <= 0) {
      setInputError(
        "Masukkan angka yang valid untuk a, r, dan n (n harus lebih besar dari 0)"
      );
      setResult(null);
      return;
    }
    ...
    const sum =
    r === 1 ? a * n : a * (1 - Math.pow(r, n)) / (1 - r);
  const series = Array.from({ length: n }, (_, i) => a * Math.pow(r, i)).join(", ");
  ...
  const newHistory = [
    ...history,
    { firstTerm: a, commonRatio: r, termNumber: n, result: newResult },
  ];
...
export default GeometricSeriesCalculator;
